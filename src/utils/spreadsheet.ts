...
export function exportTicketsToCSV(tickets: TicketRecord[]): string {
  ...
  const rows = tickets.map(t => {
    const gradeText = t.grade || t.attribute || '-';
    const classText = t.className || '-';
    const numText = t.attendanceNumber ? `${t.attendanceNumber}番` : '-';
    const peopleText = `${t.numberOfPeople || 1}名`;
    const statusText = queueLabel[t.queueStatus] || t.queueStatus;

    return [
      t.ticketNumber,
      `"${(t.timeSlot || '').replace(/"/g, '""')}"`,
      `"${(t.attribute || t.grade || '生徒').replace(/"/g, '""')}"`,
      `"${gradeText}"`,
      `"${classText}"`,
      `"${numText}"`,
      `"${(t.representativeName || t.name || '').replace(/"/g, '""')}"`,
      `"${peopleText}"`,
      `"${statusText}"`,
      `"${t.calledAt || '-'}"`,
      `"${t.completedAt || '-'}"`,
      `"${t.registeredAt ? new Date(t.registeredAt).toLocaleString('ja-JP') : '-'}"`
    ];
  });
...
export function exportTicketsToTSV(tickets: TicketRecord[]): string {
  const headers = [
    '整理番号',
    '時間帯',
    '属性',
  ...
  const rows = tickets.map(t => {
    const classText = (t.grade && t.className) 
      ? `${t.grade}${t.className}` 
      : (t.attribute || t.grade || '一般');
    const numText = t.attendanceNumber ? `${t.attendanceNumber}番` : '-';
    const peopleText = `${t.numberOfPeople || 1}名`;
    const statusText = queueLabel[t.queueStatus] || t.queueStatus;
    ...
    return [
      t.ticketNumber,
      t.timeSlot || '',
      t.attribute || t.grade || '生徒',
      classText,
      numText,
      t.representativeName || t.name || '',
      peopleText,
      statusText,
      t.calledAt || '-',
      t.completedAt || '-'
